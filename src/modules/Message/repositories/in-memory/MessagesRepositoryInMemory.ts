...
class MessagesRepositoryInMemory implements IMessagesRepository {
    ...
    async findAllUserMessagesInChat(chat_id: string, user_id: string): Promise<Either<MessageNotFoundException, MessageModel[]>> {
        const messages = this.messages.filter(message => message.chatId === chat_id && message.userId === user_id);
        if (!messages) {
    ...
    async findAllByChat(chat_id: string): Promise<Either<MessageNotFoundException, MessageModel[]>> {
        const messages = this.messages.filter(message => message.chatId === chat_id);
        if (!messages) {
            return left(new MessageNotFoundException());
        };
        return right(messages);
    ...
    async findSpecificMessage(chat_id: string, user_id: string, text: string): Promise<Either<MessageNotFoundException, MessageModel>> {
        const messageOrError = this.messages.find(message => message.chatId === chat_id && message.userId === user_id && message.text);
        if (!messageOrError) {
            return left(new MessageNotFoundException())
        }
    ...
    async findAllByUser(user_id: string): Promise<Either<MessageNotFoundException, MessageModel[]>> {
        const messages = this.messages.filter(message => message.userId === user_id);
        if (!messages) {
            return left(new MessageNotFoundException());
        };
        return right(messages);
